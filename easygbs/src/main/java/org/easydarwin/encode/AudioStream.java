...
public class AudioStream {
    ...
    public void addPusher(Pusher pusher){
        aac = PreferenceManager.getDefaultSharedPreferences(context).getInt("key-aac-codec",2);
            ...
            if (pusher != null) {
                if (aac==2) {
                    if(Audiochannel == AudioFormat.CHANNEL_IN_MONO){
                        ...
                        pusher.setAFormat(Device.AUDIO_CODEC_AAC, samplingRate, 2, 16);
                    }
                } else {
                    if(Audiochannel == AudioFormat.CHANNEL_IN_MONO){
                        pusher.setAFormat(Device.AUDIO_CODEC_G711U, samplingRate, 1, 16);
                    }else if(Audiochannel == AudioFormat.CHANNEL_IN_STEREO){
                        pusher.setAFormat(Device.AUDIO_CODEC_G711U, samplingRate, 2, 16);
                    }
                }
            }
    ...
    */
        ...
        mThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    int bufferSize = AudioRecord.getMinBufferSize(samplingRate,Audiochannel,AudioFormat.ENCODING_PCM_16BIT);

                    mAudioRecord = new AudioRecord(MediaRecorder.AudioSource.MIC, samplingRate,Audiochannel, AudioFormat.ENCODING_PCM_16BIT, bufferSize);
                    ...
                    int audiocoderate=PreferenceManager.getDefaultSharedPreferences(context).getInt("audiocoderate",16);
    ...
    public String getFileTime(){
        ...
        String str=df.format(curTime).toString();
        return str;
    }
}
